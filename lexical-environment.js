/**
 * LexicalEnvironment - термин, который используется для определения связей между
 * идентификаторами и отдельными переменными и функциями, на основе лексической
 * вложенности ES-кода.
 *
 * состоит из
 *
 * Environment Record - "обьект", который содержит все обьявления в текущей области
 *
 * outer - ссылка на внешний обьект, если его нет - null (ссылается туда, где была ОБЬЯВЛЕННА ф-я, не важно, как вызвана)
 *
 * идентификатором - имя переменной или ффункции
 * переменной - ссылка на конкретный обьект (ф-ю)
 *
 ...
 * Начало работы
 *
 * перед началом работы, код анализируется и разбивается на токены (лексемы)
 * далее анализ AST - заполнение Scope
 *
 * примерное представление скоупа
 *
