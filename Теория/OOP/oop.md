...
Это свойство родственных обьектов (имеющих общего родителя) - процесс переопределения свойсвти родителя у дочерних елементов таким образом, что бы общий тип остался не изменным. Например у родителя есть пустой метод, в наследниках мы этот метод переопределяем, но общий тип результата остался равным родителю
