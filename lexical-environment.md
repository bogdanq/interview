...
Это функция первого класса, в теле которой присутствуют ссылки на внешние переменные. (функция ссылается на свободные переменные из своей области видимости). Все функции в js являются замыканиями, потому что они запоминают внешние переменные через свойство [[ Environment ]].
